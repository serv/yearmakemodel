...
export function CommentThread({ comment, postId }: { comment: Comment, postId: string }) {
    const [isReplying, setIsReplying] = useState(false);

    async function handleReply(formData: FormData) {
        const content = formData.get('content') as string;
        await createComment("00000000-0000-0000-0000-000000000000", postId, content, comment.id); // Mock user ID
        setIsReplying(false);
    }
