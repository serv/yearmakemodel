...
export async function createPost(
  ...
  data: {
    title: string;
    content: string;
    tags: { year?: string; make?: string; model?: string; trim?: string; drivetrain?: string; transmission?: string };
  }
) {
  ...
  if (tagNames.length > 0) {
      const foundTags = await db.select().from(tags).where(inArray(tags.name, tagNames));
...
export async function getPosts(filters?: { make?: string[], year?: string[], model?: string[] }) {
